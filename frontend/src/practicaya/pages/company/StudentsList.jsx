...
import { Search, ChevronDown, BookOpen, MapPin, MessageSquare } from 'lucide-react';
...
const StudentsList = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        const res = await fetch('/practicaya/api/company/students', {
  ...
  const handleSaveCandidate = async (studentId, isSaved) => {
    ...
    try {
      const url = isSaved 
        ? `/practicaya/api/company/saved_candidates?student_id=${studentId}`
        : '/practicaya/api/company/saved_candidates';
              ...
              <div key={candidate.id} className="card candidate-card" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)', fontSize: '12px' }}>
                    ...
                    <span>{candidate.semestre ? `Semestre ${candidate.semestre}` : 'Semestre N/A'} - {candidate.universidad || 'Universidad'}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)', fontSize: '12px' }}>
                    <MapPin size={14} color="#999" />
                    <span>{candidate.ubicacion || 'Ubicación no especificada'}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: 'auto', paddingTop: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                    ...
                    <button 
                      ...
                      onClick={() => navigate(`/empresa/estudiantes/${candidate.id}`, { state: { from: '/empresa/estudiantes' } })}
                    >
                      Ver perfil
                    </button>
                  </div>
