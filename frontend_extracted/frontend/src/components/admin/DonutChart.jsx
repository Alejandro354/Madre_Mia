 ...
 * Reutiliza una paleta fija coherente con el resto de la app.
 */
...
export default function DonutChart({ data = [] }) {
  ...
  // Ángulo de inicio acumulado de cada segmento, calculado de forma funcional
  // (la suma de los valores anteriores) para no reasignar variables en render.
  ...
  return (
    <div className="donut">
      <svg viewBox={`0 0 ${size} ${size}`} width="240" height="240" role="img">
        ...
        <text x={cx} y={cy - 4} textAnchor="middle" fontSize="32" fontWeight="700" fill="var(--text-primary)">
          {total}
        </text>
        <text x={cx} y={cy + 20} textAnchor="middle" fontSize="15" fill="var(--text-secondary)">
          Total
        </text>
      </svg>
