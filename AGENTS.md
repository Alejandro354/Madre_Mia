...
**Estados de carga (skeletons)**: mientras una pantalla trae datos de la BD ya no se muestra el texto "Cargando...". `src/components/Skeleton.jsx` expone primitivos con shimmer (`SkeletonLine`/`SkeletonBlock`/`SkeletonCircle`/`SkeletonChip`) y `src/components/Loaders.jsx` los componentes por pantalla (`BrandLoader`, `VacanciesSkeleton`, `ApplicationsSkeleton`, `FavoritesSkeleton`, `VacancyDetailSkeleton`, `ProfileSkeleton`, `PortfolioSkeleton`). Cada página hace `if (loading) return <XxxSkeleton />`; `ProtectedRoute` usa `BrandLoader` (rayo PrácticaYa pulsante) durante el bootstrap de auth, y `Portfolio.jsx` muestra `PortfolioSkeleton` (antes devolvía `null`) al entrar a la pestaña Portafolio. Los skeletons reutilizan las clases de layout reales (`.vac-grid`, `.panel`, `.split-view`, etc.) para no saltar; el CSS de shimmer y del loader está en `App.css` (`.sk-*`, `.brand-loader-*`). No afecta la medición del `PerfNote` (depende de `loading`, no del markup).
