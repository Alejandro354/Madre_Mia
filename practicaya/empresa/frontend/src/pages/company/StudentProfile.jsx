...
import { ArrowLeft, Bookmark, MessageSquare, MapPin, Mail, Phone, Calendar, Clock, DollarSign, Download, ExternalLink, GraduationCap, Briefcase } from 'lucide-react';
      ...
      try {
        ...
        if (res.ok) {
          ...
          setCandidate({
            ...
            role: data.carrera || 'Estudiante',
            location: data.ubicacion || 'Ubicación no especificada',
            modality: 'Presencial',
            schedule: data.disponibilidad || 'Medio tiempo',
            description: data.descripcion || 'Sin descripción.',
            skills: data.habilidades ? data.habilidades.split(',').map(s=>s.trim()) : [],
            email: data.email || 'No especificado',
            phone: data.telefono || 'No especificado',
            availableFrom: data.disponibilidad || 'Inmediata',
            experience: 'Sin experiencia',
            educationLevel: data.semestre ? `Semestre ${data.semestre}` : 'Universitario',
            area: data.carrera || 'General',
            portfolio: [...(data.portfolios || []), ...(data.social_links || [])].map(item => ({
              id: item.id,
              type: item.type || 'link',
              name: item.name || item.red_social || item.url,
              url: item.url
            }))
          });
        }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', paddingBottom: '2rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', color: 'var(--text-secondary)' }}>
        ...
        <span style={{ fontSize: '14px' }}>
          Candidatos <span style={{ color: '#ccc', margin: '0 0.5rem' }}>/</span> <span style={{ color: 'var(--text-primary)', fontWeight: '600' }}>{candidate.name}</span>
        </span>
      </div>
          ...
          <div className="card" style={{ padding: '2rem' }}>
            ...
            <div style={{ marginBottom: '2rem' }}>
              ...
              <div className="grid-4col" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-primary)', fontWeight: '500', fontSize: '14px' }}>
                    <Clock size={18} color="var(--text-secondary)" /> Disponibilidad
                  </div>
                  <span style={{ color: 'var(--text-secondary)', fontSize: '12px', paddingLeft: '1.6rem' }}>{candidate.schedule}</span>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-primary)', fontWeight: '500', fontSize: '14px' }}>
                    <Calendar size={18} color="var(--text-secondary)" /> Inicio
                  </div>
                  <span style={{ color: 'var(--text-secondary)', fontSize: '12px', paddingLeft: '1.6rem' }}>{candidate.availableFrom}</span>
                </div>
              </div>
            </div>
          ...
          <div className="card" style={{ padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                ...
                <span style={{ fontWeight: '500', color: 'var(--text-primary)' }}>{candidate.availableFrom}</span>
              </div>
              <div style={{ width: '100%', height: '1px', background: 'var(--border-color)' }} />

              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                <span style={{ color: 'var(--text-secondary)' }}>Experiencia</span>
                <span style={{ fontWeight: '500', color: 'var(--text-primary)' }}>{candidate.experience}</span>
              </div>
              <div style={{ width: '100%', height: '1px', background: 'var(--border-color)' }} />
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                ...
                <span style={{ fontWeight: '500', color: 'var(--text-primary)' }}>{candidate.area}</span>
              </div>
            </div>
          </div>
