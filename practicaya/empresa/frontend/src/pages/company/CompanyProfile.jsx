...
const CompanyProfileView = () => {
  ...
  const handleCreateVacancy = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/company/vacancies`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (res.ok) {
        ...
        setVacancyForm({ cargo: '', descripcion: '', requisitos: '', beneficios: '', ubicacion: '', modalidad: 'Presencial', tipo_contrato: 'Tiempo completo' });
        fetchVacancies();
      } else {
        const errData = await res.json();
        alert(`Error al publicar la vacante: ${errData.msg || res.statusText}`);
      }
    } catch (error) {
      console.error(error);
      alert('Error de red al publicar la vacante');
    } finally {
      ...
      {isVacancyModalOpen && (
        <div className="modal-overlay" onClick={() => setIsVacancyModalOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '600px' }}>
            ...
            <div className="modal-header">
              <h3 className="modal-title">Crear Vacante</h3>
              <button className="modal-close" onClick={() => setIsVacancyModalOpen(false)}>
                <X size={20} />
              </button>
            </div>

            <form id="create-vacancy-form" onSubmit={handleCreateVacancy} className="modal-body">
