...
const SavedCandidates = () => {
  ...
  const fetchSavedCandidates = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/company/saved_candidates', {
  ...
  const handleRemoveCandidate = async (studentId, e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:5000/api/company/saved_candidates?student_id=${studentId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {loading ? (
          ...
          candidates.map((candidate) => {
            ...
            const estadoBg = candidate.estado === 'En proceso' ? 'rgba(245, 158, 11, 0.1)' : 'rgba(16, 185, 129, 0.1)';
            const estadoColor = candidate.estado === 'En proceso' ? '#F59E0B' : '#10B981';

            return (
              <div key={candidate.id} className="saved-table-row" style={{
                display: 'grid',
                gridTemplateColumns: '3fr 1.5fr 1.5fr 1fr 1.5fr 1fr 1fr',
                gap: '1rem',
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow = '0 4px 12px rgba(238, 43, 71, 0.1)';
              }}
              onMouseLeave={(e) => {
